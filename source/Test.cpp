...
void Test::getPic(cv::Mat image,float width,float height,int circle_size, string path){
    ...
    // ofstream worldpoint("../debug/worldpoint.txt");
    ...
    cv::imwrite("../debug/test/single.png",image1);
...
void Test::getCircle(cv::Mat image_color,float width,float height,string path){
...
int main(int argc, char const *argv[])
{
    ...
    string path = "../debug/"+std::to_string(width*20.0).substr(0,to_string(width*20.0).size()-5) 
    ...
    // cv::Mat image_back = cv::imread("../debug/"+pic_name+"-b.jpg",1);
    cv::Mat image_front = cv::imread("../debug/"+pic_name+"-f.jpg",1);
    cv::Mat image_left = cv::imread("../debug/"+pic_name+"-l.jpg",1);
    ...
    test.getPic(image,width,height,circle_size,path);
    // test.getCircle(image_back,width*20,height*20,path_back);
    test.getCircle(image_left,width*20,height*20,path_left);
    test.getCircle(image_front,width*20,height*20,path_front);
